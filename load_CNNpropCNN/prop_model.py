...
from load_CNNpropCNN import utils
from load_CNNpropCNN.unet import UnetGenerator, init_weights, norm_layer
from load_CNNpropCNN import prop_ideal
from load_CNNpropCNN.prop_submodules import Field2Input, Output2Field, Conv2dField,\
...
class CNNpropCNN(nn.Module):
    ...
    def forward(self, field):
        ...
        return center_crop(output_field, self.roi_res)
...
class CNNpropCNNprop(nn.Module):
    """
    A parameterized model with CNNs
    ...
    def forward(self, field):
        ...
        if self.prop_slm_wrp is not None:
            wrp_field = self.prop_slm_wrp(slm_field)  # Propagation from SLM to Intermediate plane.
        if self.target_cnn is not None:
            wrp_field = self.target_cnn(wrp_field)  # Applying CNN at Target planes.
        if self.prop_wrp_target is not None:
            target_field = self.prop_wrp_target(wrp_field)  # Propagation from Intermediate plane to Target planes.
