    ...
    def __init__(self, outer_nc, inner_nc, input_nc=None,
        ...
        if outermost:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
                                        ...
                                        padding=1)
            # upconv_up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            # upconv_conv1 = nn.Conv2d(inner_nc * 2, outer_nc, kernel_size=3, stride=1, padding='same')
            # upconv_conv2 = nn.Conv2d(outer_nc, outer_nc, kernel_size=3, stride=1, padding='same')
            down = [downconv, downrelu]
            # up = [upconv_up, upconv_conv1, upconv_conv2]
            ...
            # upconv_up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            # upconv_conv1 = nn.Conv2d(inner_nc, outer_nc, kernel_size=3, stride=1, padding='same')
            # upconv_conv2 = nn.Conv2d(outer_nc, outer_nc, kernel_size=3, stride=1, padding='same')
            if norm_layer is not None:
                down = [downconv, downnorm, downrelu]
                up = [upconv, upnorm, uprelu]
                # up = [upconv_up, upconv_conv1, upconv_conv2, upnorm, uprelu]
            else:
                down = [downconv, downrelu]
                up = [upconv, uprelu]
                # up = [upconv_up, upconv_conv1, upconv_conv2, uprelu]
