...
def decompose_depthmap_equal(depthmap_virtual_D, depth_planes_D):
    """ decompose a depthmap image into a set of masks with depth positions (in Diopter) """
    ...
    depth_value = depthmap_virtual_D.flatten()
    depth_value_sort, depth_value_sort_idx = depth_value.sort(stable=True)
    thresholds = list(range(0, len(depth_value), len(depth_value)//num_planes))
    thresholds.append(len(depth_value))
    sort_idx_2d = unravel_index(depth_value_sort_idx, depthmap_virtual_D.shape[-2:])
    for i in range(num_planes):
        masks[:, i, sort_idx_2d[0,thresholds[i]:thresholds[i+1]], \
              sort_idx_2d[1,thresholds[i]:thresholds[i+1]]] = 1
